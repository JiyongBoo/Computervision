...
def rnn_backward(dh, cache):
    ...
    for i in reversed(range(T)):
        if i == 0:
            ...
            prev_h = h[:,i-1,:]
        dx[:,i,:], dprev_h, prev_dWx, prev_dWh, prev_db = rnn_step_backward(dnext_h+dh[:,i,:],cache = (x[:,i,:], prev_h, h[:,i,:], Wx, Wh))
        dnext_h = dprev_h
        dWx += prev_dWx
        dWh += prev_dWh
        db += prev_db
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    _, H = prev_h.shape
    i = sigmoid(np.matmul(x, Wx[:,:H]) + np.matmul(prev_h, Wh[:,:H]) + b[:H])
    f = sigmoid(np.matmul(x, Wx[:,H:2*H]) + np.matmul(prev_h, Wh[:,H:2*H]) + b[H:2*H])
    o = sigmoid(np.matmul(x, Wx[:,2*H:3*H]) + np.matmul(prev_h, Wh[:,2*H:3*H]) + b[2*H:3*H])
    g = np.tanh(np.matmul(x, Wx[:,3*H:]) + np.matmul(prev_h, Wh[:,3*H:]) + b[3*H:])
    next_c = f*prev_c + g*i
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    ds = dnext_h * o * (1 - tanh_next_c**2) + dnext_c
    dprev_c = ds*f
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    N, H = h0.shape
    h = np.zeros((N,T,H))
    i,f,o,g,c = np.zeros((N,T,H)), np.zeros((N,T,H)), np.zeros((N,T,H)), np.zeros((N,T,H)), np.zeros((N,T,H))

    prev_h = h0
    prev_c = 0
    for k in range(T):
        next_h, next_c , stepcache = lstm_step_forward(x[:,k,:],prev_h,prev_c,Wx,Wh,b)
        *_, i[:,k,:],f[:,k,:],o[:,k,:],g[:,k,:],c[:,k,:] = stepcache
        h[:,k,:] = next_h
        prev_h = next_h
        prev_c = next_c

    cache = (x, h0, h, Wx, Wh, i, f, o, g, c)
...
def lstm_backward(dh, cache):
    ...
    for k in reversed(range(T)):
        if k == 0:
            ...
            prev_h = h[:,k-1,:]    
            prev_c = c[:,k-1,:]                          
        dx[:,k,:], dprev_h, dprev_c, prev_dWx, prev_dWh, prev_db = lstm_step_backward(dnext_h+dh[:,k,:],dnext_c,cache = (x[:,k,:], prev_h, prev_c, Wx, Wh,
                                                                                                                         i[:,k,:],f[:,k,:],o[:,k,:],g[:,k,:],c[:,k,:]))
        dnext_h = dprev_h
        dnext_c = dprev_c
        dWx += prev_dWx
