...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        projected_features = projected_features.unsqueeze(1).repeat(1, T, 1) # (N, T, W)
        tgt_mask = torch.tril(torch.ones((T, T))).bool()
        
        output, att_map = self.transformer(caption_embed, projected_features, tgt_mask=tgt_mask)   # (N, T, W)

        scores = self.output(output)  # (N, T, V)
        self.register_buffer('att_map', att_map)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory, tgt_mask=None):
        ...
        # layer norm).
        tgt2,att_map = self.self_attn(query=tgt, key=tgt, value=tgt, attn_mask=tgt_mask)
