...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        div_term = torch.arange(0,embed_dim,step=2)

        pe[0,:,0::2] = torch.sin(pos/10000**(div_term/embed_dim))
        pe[0,:,1::2] = torch.cos(pos/10000**(div_term/embed_dim))
        pe = pe.double()
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        query = query.view(N,-1,self.n_head, self.head_dim).transpose(1,2)
        key = key.view(N,-1,self.n_head, self.head_dim).transpose(1,2)
        value = value.view(N,-1,self.n_head, self.head_dim).transpose(1,2)
        ...
        if attn_mask is not None:
          score = score.masked_fill(attn_mask==False, -1e10)
        
        score = nn.Softmax(dim=-1)(score)
        output = self.attn_drop(score)
        output = torch.matmul(output, value)
        
        output = output.transpose(1,2)
        output = output.contiguous()
        output = output.view(N, -1, self.emd_dim)
